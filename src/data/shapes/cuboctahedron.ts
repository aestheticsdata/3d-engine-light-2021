...
const indicesWhere = (predicate: (vertex: number[]) => boolean) =>
  vertices.reduce<number[]>((indices, vertex, index) => {
    if (predicate(vertex)) {
      indices.push(index);
    }
...
AXES.forEach((axis) => {
  SIGNS.forEach((sign) => {
    faces.push(indicesWhere((vertex) => vertex[axis] === sign));
  });
});
...
const cuboctahedron: Object3D = { points, triangles };
