...
// pushed in through setField, which is why it doubles as the sanity check that
// the shared state layer really is wired to engine state.
//
...
export const createStatusBar = () => ({
  setRunState: (isPlaying: boolean) => {
    setField("statusLabel", isPlaying ? "RUNNING" : "PAUSED");
  },
  ...
  setSelected: (primitive: string) => {
    setField("selectedId", sceneObjectId(primitive));
  },

  setMode: (wireframeEnabled: boolean) => {
    setField("shadingMode", modeLabel(wireframeEnabled));
  },

  setTexture: (object3D: Object3D) => {
    setField("texLabel", texLabel(object3D));
  },
});
