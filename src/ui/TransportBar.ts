// The transport: the play/pause mounts and the REC dot.
//
// Each control has two mounts — the desktop strip and the mobile header —
// because there is one DOM tree and no control can be in two parents at once.
// Binding by attribute rather than by id keeps that one handler and no second
// code path, which is also why this class holds node lists rather than elements.
//
// RESET left with E8a. It carries data-action, and ActionRegistry now binds
// every node that does; a class binding it here as well would have been a second
// listener on the same button and two resets per click.
