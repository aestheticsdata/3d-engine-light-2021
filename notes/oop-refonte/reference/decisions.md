...
reopen: the texture derivation *was* converted (T06 / COS-377, and is now the class
[`MaterialSummary`](../../../src/ui/MaterialSummary.ts)) while `sceneObjectId.ts` was
not, and both were standalone pure-function modules with exactly two call sites. The
difference is that the texture derivation had two exports bound by a shared invariant, a
real map/filter/Set/dedupe pipeline that ran twice over the same object, and an unchecked
...
| -- | -- | -- | -- |
| `src/data/shapeInfo.ts` | 273 | inert data (D1a) | A prose catalogue keyed by shape name: two interfaces at `:1-19`, one `Record` literal at `:21-271`, `export default` at `:273`. Zero functions, zero loops, zero imports. Its only behavioural use is the caller's lookup, now `Main.ts:303` feeding `ShapeStoryPanel.show`. |
...
| `src/ui/tooltip.ts` | 81 | conformant, legacy name kept | The only class `src/ui/` had before the refonte, and the cited precedent for R9. The basename does not share the class's stem (`FollowCursorTooltip`), but its importer — now `RenderPipelinePanel.ts:20` — and `styles/components/tooltip.css` both pair to the specifier by name. R13 carves it out explicitly. |
...
| `src/data/data.ts` | 36 | inert data (D1a) | Fourteen imports, one registry literal at `:19-34`, `export default` at `:36`. No functions. The lookups that might motivate a registry class are caller-side — `Main.ts:126` takes the whole registry and indexes it by primitive key — and closing the key set is carved out as **COS-390**. |
...
It was listed as one in T01's original acceptance checklist. That was wrong and the
criterion was corrected: **the list is ten files, not eleven.**

The old root-level `controls.ts` was genuinely conformant — bare `class Controls` at
`:1`, `export default` last at `:53`, four explicitly-`public` methods — but T18
(COS-373) **deleted the file** and rebuilt its four methods as
[`SliderBank`](../../../src/ui/SliderBank.ts) and
[`PrimitivePicker`](../../../src/ui/PrimitivePicker.ts). Its first acceptance line was
"`controls.ts` no longer exists", and it does not.
...
**zero code hits** today — the four matches are the words "static", "protected" and
"implements" inside comments (`UIPanel.ts:15`, `scene/SceneRowView.ts:119`,
`builders/Icosahedron.ts:19-20`), three of which exist to record why the keyword was not
used. That was the baseline and the epic did not change it.
...
| Baseline | Command | At recording | Target at epic end | Measured at epic end |
| -- | -- | -- | -- | -- |
| Factory/helper default exports in the layers being converted | `grep -rn '^export default [a-z]' src/ui/ src/primitives/ src/rendering/ src/animations/` | **5** (was 7 before T01) | 0 | **0** ✓ |
| Repo-wide `export default <lowercase>` | `grep -rn '^export default [a-z]' src/` | **21** (was 23) | 16 — the `export default sphere;` tails D1a keeps | **16** ✓ |
| `static` / `abstract` / `protected` / `implements` | `grep -rnE '\b(static\|abstract\|protected\|implements)\b' --include='*.ts' src/` | **0 code hits** | 0 — unchanged (D4) | **0 code hits** ✓ |
| Module-scope `let` | `grep -rn '^let ' --include='*.ts' src/` | **3** — `torusKnot.ts:94,134`, `rhombicTriacontahedron.ts:105` | 0 | **0** ✓ |
| `tsc --noEmit` | `pnpm run typecheck` | **exit 0, zero errors** | exit 0 | **exit 0** ✓ |
| behaviour harness | `pnpm test` | **18 tests, 3 files, green** (recorded at T03, the first ticket that could) | green, plus the fps meter suite T13 owes | **33 tests, 5 files, green** ✓ |

The **At recording** column is what the tree measured before any conversion ticket; it is
kept as written rather than refreshed, because its job is to be the thing the last column
is compared against. The final column was measured at T33, with every other sub-issue
closed. The harness grew past the four modules T03 scoped: T13 added the fps meter suite
it owed, and T29 added the projection suite.
...
`src/app/Main.ts` was **282 code lines** after T19 and is **286** today, so it does not
meet R17's ~160-line split. It is exempt, and the exemption is narrow: it applies to the one class whose job
