...
- `src/ui/UIStateStore.ts` — `snapshot()` / `hydrate(partial)` over the existing slice registry
...
- [ ] SAVE PRESET downloads JSON; LOAD restores every slider, both flags, the primitive and every registered `UIStateStore` slice in one repaint; loading the same file twice in a row works
