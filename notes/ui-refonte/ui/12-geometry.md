...
| CULLED | integer, orange | Real, with a caveat **widened by COS-236 (de-mock E2)**: the near plane means `Triangle.render()` now also returns `false` for a triangle outside the view volume, so the subtraction is culled + clipped + degenerate-UV skips, and with culling OFF the row reads 0 while clipped triangles are still going missing. Separating the three is de-mock E6's. Original wording follows. `submitted − renderedTriangles` when `backfaceCullingEnabled` is on, else 0. `Triangle.render()` returns `false` at the backface test (src/primitives/Triangle.ts L125–L127) **and** at the degenerate-UV guard for textured triangles (L186–L189), and `Mesh.renderMesh()` (src/primitives/Mesh.ts L22–L26) only increments its counter on a truthy return, so the subtraction is culled + degenerate-UV skips. Degenerate UVs are not expected in the current registry, but the cube is textured, so the path is reachable — note the approximation in a code comment rather than claiming exactness. Requires exposing the count: `Mesh.triangles` is `private readonly`, so add `public get triangleCount(): number { return this.triangles.length; }` to `src/primitives/Mesh.ts`. |
...
- [ ] CULLED reads 0 when backface culling is off and equals submitted − drawn when it is on, verified against a shape with a known registry count, with the degenerate-UV caveat recorded in a code comment — COS-236 widened that comment to cover near/far clipping as well
